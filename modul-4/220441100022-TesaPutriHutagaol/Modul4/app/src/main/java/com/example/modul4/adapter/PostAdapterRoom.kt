...
class PostAdapterRoom(private var postList: List<PostDatabase>) :
    ...
    class PostViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val postLikeImageView: ImageView = itemView.findViewById((R.id.suka))
        //btn
    ...
    override fun onBindViewHolder(holder: PostViewHolder, position: Int) {
        ...
        holder.btnLike.setOnClickListener {
            if (!isLiked) {
                data.like += 1
                holder.postLike.text = data.like.toString()
                holder.postLikeImageView.setImageResource(R.drawable.likemerah) // mengubah sumber daya ImageView menjadi versi merah
                isLiked = true // Setelah tombol "like" ditekan, tandai bahwa sudah ditekan
            } else {
                data.like = 0
                holder.postLike.text = data.like.toString()
                holder.postLikeImageView.setImageResource(R.drawable.loveicon) // mengubah sumber daya ImageView menjadi versi putih
